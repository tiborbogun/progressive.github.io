...
class sidebar_generator {
	...
	function admin_menu(){
		add_theme_page('Sidebar', 'Sidebar', 'manage_options', 'multiple_sidebars', array('sidebar_generator','admin_page'));
	}
	...
	function admin_page(){
		...
		<script>
			function remove_sidebar_link(name,num){
				answer = confirm("Jesteś pewien, że chcesz skasować " + name + "?\nJednocześnie usuniesz wszystkie widgety dodane do tego sidebaru.");
				if(answer){
					//alert('AJAX REMOVE');
			...
			function add_sidebar_link(){
				var sidebar_name = prompt("Nazwa sidebaru:","");
				//alert(sidebar_name);
				add_sidebar(sidebar_name);
			}
		</script>
		<div class="wrap">
			<h2>Generator Sidebarów</h2>
			<p>
				Nazwa sidebaru służy tylko Tobie. Nie będzie widoczna osobom odwiedzającym stronę.
				Klasa CSS jest dopisana do każdego sidebaru, w celu dostosowania sidebaru z innym wyglądem.
			</p>
			<br />
			<div class="add_sidebar">
				<a href="javascript:void(0);" onclick="return add_sidebar_link()" title="Add a sidebar">+ Dodaj sidebar</a>
			</div>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				...
				<tr class="<?php echo $alt?>">
					...
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo $sidebar; ?>',<?php echo $cnt+1; ?>);" title="Usuń ten sidebar">usuń</a></td>
				</tr>
				<?php
						$cnt++;
					}
					...
					?>
					<tr>
						<td colspan="3">Brak sidebarów</td>
					</tr>
					<?php
				}
				?>
		...
		<div id="sbg_box" class="postbox " >
			...
			<div class="inside">
				<div class="sbg_container">
					...
					<p>
						Wybierz sidebar, który chcesz aby się wyświetlił. Pamiętaj, w tym celu musisz użyć domyślnego szablonu strony.<br />
						<strong>Uwaga:</strong> Najpierw musisz stworzyć sidebar w meu Wygląd > Sidebar. 
					</p>
					<ul>
					<?php 
